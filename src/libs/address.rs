...
pub fn get_address(address_id: i32) -> Result<Json<Address>, status::NotFound<String>> {
    ...
    let address = _get_address(&conn, address_id);
...
fn _get_address(conn: &PgConnection, address_id: i32) -> Option<Address> {
    address::table
        .find(address_id)
        .first::<Address>(conn)
        .optional()
...
pub fn get_addresses() -> Json<Vec<Address>> {
    let conn = PgConnection::establish(DATABASE_URL)
        .unwrap_or_else(|_| panic!("Error connecting to {}", DATABASE_URL));
    let addresses = _get_addresses(&conn);
    Json(addresses)
}

fn _get_addresses(conn: &PgConnection) -> Vec<Address> {
    address::table
        .load::<Address>(conn)
        .unwrap()
}
...
pub fn create_address(mut address: Form<NewAddress>) -> Result<Json<Address>, status::BadRequest<String>> {
    ...
    let new_address = _create_address(&conn, address.into_inner());
...
fn _create_address(conn: &PgConnection, address: NewAddress) -> QueryResult<Address> {
...
pub fn update_address(address_id: i32, mut address: Form<NewAddress>) -> String {
    ...
    let updated_address = _update_address(&conn, address_id, address.into_inner());
...
fn _update_address(conn: &PgConnection, address_id: i32, address: NewAddress) -> QueryResult<Address> {
...
pub fn delete_address(address_id: i32) -> String {
    let conn = PgConnection::establish(DATABASE_URL)
        .unwrap_or_else(|_| panic!("Error connecting to {}", DATABASE_URL));
    let deleted_address = _delete_address(&conn, address_id);
...
fn _delete_address(conn: &PgConnection, address_id: i32) -> QueryResult<usize> {
